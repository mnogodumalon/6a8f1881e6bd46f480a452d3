...
import type { Touren, Buchungen, CreateTouren, CreateBuchungen } from '@/types/app';
...
export class LivingAppsService {
  // --- TOUREN ---
  static async getTouren(): Promise<Touren[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TOUREN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Touren[];
    return hydrateRecords(records, 'touren');
  }
  static async getTourenEntry(id: string): Promise<Touren | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TOUREN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Touren;
    return hydrateRecords([record], 'touren')[0];
  }
  static async createTourenEntry(fields: CreateTouren): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.TOUREN}/records`, { fields: cleanFieldsForApi(fields as any, 'touren') });
    return { ...data, record_id: data.id };
  }
  static async updateTourenEntry(id: string, fields: Partial<CreateTouren>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.TOUREN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'touren') });
    return { ...data, record_id: data.id };
  }
  static async deleteTourenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TOUREN}/records/${id}`);
  }
