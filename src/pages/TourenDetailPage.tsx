...
import { formEnhancements } from '@/config/form-enhancements/Touren';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function TourenDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Touren | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getTouren();
  ...
  async function handleUpdate(fields: Touren['fields']) {
    if (!record) return;
    await LivingAppsService.updateTourenEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteTourenEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/touren');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/touren')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/touren')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.treffpunkt ?? appLabel('touren')} />
