...
import { TourenViewDialog } from '@/components/dialogs/TourenViewDialog';
...
const TOUREN_FIELDS = [
  ...
  { key: 'guide_vorname', label: 'Vorname des Guides', type: 'string/text' },
  { key: 'guide_nachname', label: 'Nachname des Guides', type: 'string/text' },
  { key: 'max_teilnehmer', label: 'Maximale Teilnehmerzahl', type: 'number' },
  { key: 'anmerkungen_tour', label: 'Anmerkungen zur Tour', type: 'string/textarea' },
];
const BUCHUNGEN_FIELDS = [
  { key: 'tour', label: 'Tour', type: 'applookup/select', targetEntity: 'touren', targetAppId: 'TOUREN', displayField: 'treffpunkt' },
  { key: 'gast_vorname', label: 'Vorname des Gastes', type: 'string/text' },
  { key: 'gast_nachname', label: 'Nachname des Gastes', type: 'string/text' },
  ...
  { key: 'anmerkungen_buchung', label: 'Anmerkungen zur Buchung', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'touren', pascal: 'Touren' },
  { key: 'buchungen', pascal: 'Buchungen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('touren');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'touren': return (data as any).touren as Touren[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'buchungen':
        lists.tourenList = (data as any).touren ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'buchungen' && fieldKey === 'tour') {
      const match = (lists.tourenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.treffpunkt ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'touren': return TOUREN_FIELDS as any[];
        case 'buchungen': return BUCHUNGEN_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'touren': return {
        create: (fields: any) => LivingAppsService.createTourenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTourenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTourenEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'touren' || dialogState?.entity === 'touren') && (
        <TourenDialog
          open={createEntity === 'touren' || dialogState?.entity === 'touren'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'touren' ? handleUpdate : (fields: any) => handleCreate('touren', fields)}
          defaultValues={dialogState?.entity === 'touren' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'buchungen' || dialogState?.entity === 'buchungen') && (
        <BuchungenDialog
          ...
          tourenList={(data as any).touren ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Buchungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Buchungen']}
        />
      )}
      {viewState?.entity === 'touren' && (
        <TourenViewDialog
          open={viewState?.entity === 'touren'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'touren', record: r }); }}
        />
      )}
